...
# Introduction (Лицо - Person)

See the content of the this section in census data:

- [In Russian](http://research.muuankarski.org/census/year2002/content/person_ru.html)
- [In English](http://research.muuankarski.org/census/year2002/content/person_en.html)
...
```{rcenMosObPersAge}
raw <-  read.csv("data/DoBMoscowOb.csv", sep=";", skip=6)
...
region_key_moscow <-  read.csv(text = GHurl)
dat <- merge(df.l,region_key_moscow,
                        by="region")

```


```{rcenMosObPersAgePlot1}
library(ggplot2)
...
library(rgdal)
#load("RUS_adm2.RData")
shapefile <- readOGR(dsn = "../../../data/shapefiles/russia/gadm/RUS_adm_simple/", 
               layer = "RUS_adm2")
...
choro <- merge(map.df,df.map.moscow,by="ID_2")
...
```{rcenMosObPersEduPlot1, eval=FALSE}
library(ggplot2)
...
```{rcenMosObPersEduMap1, eval=FALSE}
library(rgdal)
...
```{rrcenMosObPersEduMap2, eval=FALSE}
