...
library(knitr)
library(markdown)
knit2html('censusKrasnodarpers.Rmd', 
...
```{rcenKrasdPersAgePlot1, fig.height=26}
dat <- read.csv("data/mod/ageKrasnodar.csv")
library(ggplot2)
...
```{rcenKrasdPersAgeMap2}

library(ggmap)

data <- fortify(shape_krasnodar)
...
```{rcenKrasdPersEduPlot1}
load("data/mod/eduKrasnodar.RData")
...
```{rrcenKrasdPersEduMap2}
library(ggmap)
data <- fortify(shape_krasnodar)
qmap("krasnodar novgorod", zoom = 7, maptype="roadmap") +
...
```{rcenkrasnodarPersEmpPlot1, fig.height=24}
load("data/mod/emplKrasnodar.RData")
